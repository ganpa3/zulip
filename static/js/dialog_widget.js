 ...
 *
 *      9) The submit_button is focused when the modal is shown to facilitate accessibility.
 *         Pass `disable_focus_yes_button` as true to disable this behavior in dangerous operations.
 *
 *      10) If a caller needs to run code after the modal body is added
        ...
        micromodal_opts: {
            onShow: () => {
                if (conf.focus_submit_on_open) {
                    ...
                    dialog.find(".modal__overlay").trigger("focus");
                }
            },
        },
    });

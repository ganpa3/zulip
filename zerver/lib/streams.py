...
def create_streams_if_needed(
    ...
    for stream_dict in stream_dicts:
        is_invite_only = stream_dict.get("invite_only", False)
        if acting_user is not None:
            if (
                is_invite_only
                and not acting_user.can_create_private_streams()
                or not is_invite_only
                and not acting_user.can_create_public_streams()
            ):
